...
// Baseline checksumları — cards-baris.json: baris retire edilebilir (bkz meta-kanal.md
// 2026-07-10 baris-retirement-diagnosis-only), existsSync guard.
const barisVarMi = existsSync(CARDS_BARIS_YOL)
const barisOnceki = barisVarMi ? readFileSync(CARDS_BARIS_YOL, 'utf8') : null
...
const barisSonraki = barisVarMi ? readFileSync(CARDS_BARIS_YOL, 'utf8') : null
...
// Sabit proje-id listesi YERİNE test-öncesi anlık-görüntü karşılaştırması — jenerik, herhangi
// bir projenin (baris dahil) var/yok olmasından etkilenmez; satır 170-171'deki sayı-eşitliğini
// üye-bazında güçlendirir (ID-swap senaryosunu da yakalar, salt uzunluk karşılaştırması yakalamaz).
assert(projeBilgileriOnceki.every(p => projeBilgileri.some(q => q.id === p.id)),
  'test-öncesi registry üyelerinin tamamı hâlâ mevcut (hiçbiri kaybolmadı)')

// Sentez kartlar baris cards listesinde YOK (baris'in sentez-kartlar/ dizini yok) — baris yoksa N/A
if (barisVarMi) {
  ...
  console.log('  (cards-baris.json yok — sentez-kartı-eklenmedi kontrolü N/A, atlandı)')
}
